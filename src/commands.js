...
function executeClickCommand(state, cmd, mode) {
    ...
    if (!o) {
        return 'Sorry, nothing found at r=' + coord[0] + ', c=' + coord[1];
    }
...
function executeBuildCommand(state, cmd) {
    ...
    if (!b) {
        return 'unknown building type ' + type;
    }
    ...
    if (lacking) {
        return 'sorry, not have enough resources, lacking: ' + JSON.stringify(lacking);
    }
    if (!cmd[2]) {
        return { mode: 'build ' + type };
    }

    const coord = cmd[2].split(',');
    if (!common.inbounds(coord[0], coord[1])) {
        return 'out of bounds';
    }
    const o = Object.assign({ r: +coord[0], c: +coord[1], state: -1, minedAt: common.time(b.buildTime), type: type }, b);
    if (common.collides(o, state.objects)) {
        return 'sorry, not enough space there';
    }
    const lacking2 = common.deductResources(state.resources, b.build);
    if (lacking2) {
        return 'sorry, not have enough resources, lacking: ' + JSON.stringify(lacking2);
    }
